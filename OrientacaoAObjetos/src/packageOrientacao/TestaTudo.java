package packageOrientacao;

public class TestaTudo {

    public static void main(String[] args) {
        // Instância dos objetos
        Agencia a1 = new Agencia();
        Cliente c1 = new Cliente();
        Conta co1 = new Conta();
        CartaoDeCredito cdc1 = new CartaoDeCredito();

        // Atribuindo valores aos atributos
        // dos objetos instanciados
        //c1.codigo = 1;
        
        //c1.nome = "Marcos Costa";
        
        c1.setNome("Oriunda Conengada");
        a1.numero = 1234;

        co1.numero = 1234;
        co1.saldo = 1000;
        co1.limite = 500;

        cdc1.numero = 1111;
        cdc1.dataDeValidade = "01/01/2025";
        cdc1.bandeira = "Mastercard";
        

        // Mostrando todos os objetos
        System.out.println("Agencia: " + a1.numero);
        
        //System.out.println("Código do cliente: " + c1.codigo);
        System.out.println("Nome do Cliente: " + c1.getNome());
        
        
        System.out.println("Número da Conta do Cliente: " + co1.numero);
        System.out.println("Saldo da Conta do Cliente: " + co1.saldo);
        System.out.println("Limite da Conta do Cliente: " + co1.limite);
        System.out.println("Número do Cartão de Crédito do Cliente: " + cdc1.numero);
        System.out.println("Data de Validade do Carão de Crédito: " + cdc1.dataDeValidade);
        System.out.println("Bandeira do Carão de Crédito: " + cdc1.bandeira);
    }
    
}
